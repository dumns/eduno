...
@php
    $inputId = $id ?? $name . '-' . $value ?? 'radio-' . str()->random(8);
@endphp
...
<label for="{{ $inputId }}" class="inline-flex items-center gap-2.5 cursor-pointer group {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }} {{ $class }}"
    ...
    <input
        ...
        class="h-4 w-4 border-border dark:border-border-dark bg-white dark:bg-surface-dark text-primary focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-surface-dark transition-all duration-150 cursor-pointer disabled:cursor-not-allowed"
    />
    @if ($label)
